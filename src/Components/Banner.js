import LocomotiveScroll from 'locomotive-scroll';
import React, { useEffect, useRef } from 'react';
import Slider from 'react-slick';

function Banner() {
  const scrollRef = useRef(null);

  useEffect(() => {
    const locomotiveScroll = new LocomotiveScroll({
      el: scrollRef.current,
      smooth: true
    });

    return () => {
      if (locomotiveScroll) locomotiveScroll.destroy();
    };
  }, []);

  const smoothScroll = (target, duration) => {
    const targetPosition = target.getBoundingClientRect().top;
    const startPosition = window.pageYOffset;
    let startTime = null;

    const ease = (t, b, c, d) => {
      t /= d / 2;
      if (t < 1) return c / 2 * t * t + b;
      t--;
      return -c / 2 * (t * (t - 2) - 1) + b;
    };

    const animation = currentTime => {
      if (startTime === null) startTime = currentTime;
      const timeElapsed = currentTime - startTime;
      const run = ease(timeElapsed, startPosition, targetPosition, duration);
      window.scrollTo(0, run);
      if (timeElapsed < duration) requestAnimationFrame(animation);
    };

    requestAnimationFrame(animation);
  };

  const handleScroll = (e) => {
    e.preventDefault();
    const formSection = document.querySelector('#form');
    if (formSection) {
      smoothScroll(formSection, 2000); // Adjust the duration as needed
    }
  };

  var settings = {
    infinite: true,
    speed: 500,
    fade: true,
    autoplay: true,
    autoplaySpeed: 2000
  };

  return (
    <div ref={scrollRef} className='w-full overflow-x-hidden'>
      <Slider {...settings}>
        <div className="slide-1">
          <div className="relative top-[50%] flex flex-col items-center justify-center text-center">

            <h1 className="text-white playwrite text-lg lg:text-3xl md:text-2xl">
              The Perfect Venue for Grand Festivities
            </h1>

            <div className="flex justify-center items-center gap-4 w-full mt-6">
              <button
                onClick={handleScroll}
                className="px-6 py-2.5 bg-white text-black text-sm hover:scale-110 transition-all"
              >
                ENQUIRE NOW
              </button>

              <button className="bg-[#d8ac47eb] text-black uppercase px-6 py-2 hover:scale-110 transition-all">
                <a
                  className="flex justify-center items-center text-sm"
                  href="tel:+917208185772"
                >
                  Call Now
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    stroke="currentColor"
                    className="size-6 ml-2"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z"
                    />
                  </svg>
                </a>
              </button>
            </div>
          </div>
        </div>
        <div className="slide-2">
          <div className="relative top-[50%] flex flex-col items-center justify-center text-center">

            <h1 className="text-white playwrite text-lg lg:text-3xl md:text-2xl">
              Where Every Detail Exceeds Your Expectations
            </h1>

            <div className="flex justify-center items-center gap-4 w-full mt-6">
              <button
                onClick={handleScroll}
                className="px-6 py-2.5 bg-white text-black text-sm hover:scale-110 transition-all"
              >
                ENQUIRE NOW
              </button>

              <button className="bg-[#d8ac47eb] text-black uppercase px-6 py-2 hover:scale-110 transition-all">
                <a
                  className="flex justify-center items-center text-sm"
                  href="tel:+917208185772"
                >
                  Call Now
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    stroke="currentColor"
                    className="size-6 ml-2"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z"
                    />
                  </svg>
                </a>
              </button>
            </div>
          </div>
        </div>
        <div className="slide-3">
          <div className="relative top-[50%] flex flex-col items-center justify-center text-center">

            <h1 className="text-white playwrite text-lg lg:text-3xl md:text-2xl">
              Make Your Special Day an Unforgettable One
            </h1>

            <div className="flex justify-center items-center gap-4 w-full mt-6">
              <button
                onClick={handleScroll}
                className="px-6 py-2.5 bg-white text-black text-sm hover:scale-110 transition-all"
              >
                ENQUIRE NOW
              </button>

              <button className="bg-[#d8ac47eb] text-black uppercase px-6 py-2 hover:scale-110 transition-all">
                <a
                  className="flex justify-center items-center text-sm"
                  href="tel:+917208185772"
                >
                  Call Now
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    stroke="currentColor"
                    className="size-6 ml-2"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z"
                    />
                  </svg>
                </a>
              </button>
            </div>
          </div>
        </div>
      </Slider>
    </div>
  );
}

export default Banner;
